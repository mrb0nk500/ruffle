...
impl<'gc> ActionQueue<'gc> {
    ...
    /// Gets the frontmost action from the action queue.
    pub fn front_action(&mut self) -> Option<&QueuedAction<'gc>> {
        self.action_queue
            .iter_mut()
            .rev()
            .nth(0)?
    ...
    /// Gets the backmost action from the action queue.
    pub fn back_action(&mut self) -> Option<&QueuedAction<'gc>> {
        self.action_queue
            .iter_mut()
            .rev()
            .nth(0)?
